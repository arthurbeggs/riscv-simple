...
.macro M_SetEcall
 	la t6,exceptionHandling	# carrega em t6 o endere�o base das rotinas do sistema ECALL
...
exceptionHandling:  
	...
	addi sp sp -8 # salva os 2 registradores utilizados para comparar utval, caso seja um ecall, eles devem retornar antes de qualquer coisa
	sw s10 0(sp)
	sw t0 4(sp)
	...
	addi t0 zero 8
	beq t0 s10 ecallException  # Ecall

	addi sp sp 8 # se n�o for ecall, apenar voltar o sp para evitar vazamento de mem�ria	
	bne t0 s10 errorExceptions # Exce��o de causa
...
errorExceptions:
	...
	csrrw, s11 67, zero # le a causa da exce��o e salva em tp
	...
	# Imprime o pc em que a exce��o ocorreu
	la a0 PC 
	li a1 0
	li a2 12
	li a3 0x000c0ff
	...
	addi sp sp 264 # desempilha tudo antes do exit
...
ecallException:  
    ...
    lw s10 0(sp) # retorna os registradores utilizados para checar o valor de utval
    lw t0 4(sp)  
    addi sp sp 8
